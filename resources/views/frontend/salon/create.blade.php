...
<link rel="stylesheet" href="{{ asset('pages-css/book-page.css') }}?v={{ filemtime(public_path('pages-css/book-page.css')) }}">
@endsection
...
<script>
        ...
        function fetchServicesByGroup(serviceGroupId) {
            showLoader();
            fetch(`/api/services/${serviceGroupId}/${selectedData.branch}/bookings`)
                .then(response => response.json())
                .then(data => {
        ...
        function fetchStaffMembers(branchId , subserve) {
            showLoader();
            fetch(`/api/staff?branch_id=${branchId}&service_id=${subserve}`)
                .then(response => response.json())
                .then(data => {
                ...
                selectedData.services.forEach(service => {
                    if (service.subServices && service.subServices.length > 0) {
                        service.subServices.forEach(sub => {
                            ...
                            card.innerHTML = header + detailsMarkup;
                            if (deleteBtn) {
        ...
        function getBookingSummaryLocation() {
            return selectedData.branchName || '';
        }
                        ...
                        <div class="booking-summary-header">
                            <div>
                                ...
                                <p class="booking-summary-subtitle">
                                    ${lang === 'ar'
                                        ? 'راجعي الخدمات المختارة والمواعيد قبل إتمام العملية أو إضافتها إلى السلة.'
                                        : 'Review the selected services and schedule before paying or adding them to the cart.'}
                                </p>
                            </div>
