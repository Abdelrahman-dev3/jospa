...
export const CATEGORY_LIST = ({ branch_id = '' } = {}) => {return {path: `categories/index_list?branch_id=${branch_id}`, method: 'GET'}}
export const SERVICE_LIST = ({ branch_id = '', category_ids = [] }) => {
  const categoryParam = Array.isArray(category_ids) ? category_ids.join(',') : category_ids
  return {path: `${MODULE}/services-index_list?branch_id=${branch_id}&category_ids=${encodeURIComponent(categoryParam ?? '')}`, method: 'GET'}
}
